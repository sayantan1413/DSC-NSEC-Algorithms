  ...
  //write your code here
   int n,c=0;
    ...
    if(n>2)
    {
        while(c!=n)
        {
            //If it is a number then continue the loop
           if(isdigit(str[c]) == true)
           {
               c++;
               continue;
           }
           //if it is an alphabet then change the character to upper or lower case and vice versa one by one
           else
           {
               str=new_str;
               convert(str,c);
    ...
    //In the last check make all the characters in uppercase and print
